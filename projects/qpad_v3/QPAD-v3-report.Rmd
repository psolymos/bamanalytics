...
* `DSLS`: days since local spring (based on date of survey relative to 30-year average of the local start of the growing season [2], standardized as $x/365$),
* `JDAY2`, `TSSR2`, `DSLS2`: quadratic terms ($x^2$) for the above variables.

The following model formulas were used as part of the right-hand side of the
...
The reason we switched from the Canadian land cover product (as described in [1])
...
We are providing parameter estimates for the following bird species:
